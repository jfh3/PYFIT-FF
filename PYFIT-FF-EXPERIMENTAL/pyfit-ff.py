	...
	if OBJECTIVE_FUNCTION == 'group-targets':
		if UNWEIGHTED_NEGATIVE_ERROR:
			...
			def get_loss():
				...
				per_group_diff_squared_avg = per_group_diff_squared_sum / group_sizes
				per_group_rmse             = torch.sqrt(per_group_diff_squared_avg)
				loss                       = (SUBGROUP_ERROR_COEFFICIENT*(group_target_array - per_group_rmse)**2).sum()
				last_loss  = loss.cpu().item()
				return loss
	else:
		def get_loss():
...
if __name__ == '__main__':
	...
	# At We want to run the training process once for every group.
	# At each iteration, create a folder fo the outputs, reconfigure
	# the global system parameters and run training with that group
	# set to a lower error than the rest. Repeat for each group and
	...
	#     SUBGROUP_ERROR_COEFFICIENT = 0.5
	#     SUBGROUP_ERROR_COEFFICIENT = 0.1
	#     SUBGROUP_ERROR_COEFFICIENT = 0.025
	#
	# Since there are 49 groups, it will probably be 4-5 hours
	# per configuration to run the whole process. (25 hours total)

	# Load a list of groups from the training file so we know
	...
	# Now we know what group names we are working with.
	error_coefficients = [0.5]

	# Prepare the output directory.

	now               = datetime.now()
	dt_string         = '%4i-%02i-%02i:%02i:%02i:%02i'%(now.year, now.month, now.day, now.hour, now.minute, now.second)
	current_directory = 'output_' + dt_string + '/'
	...
	def set_output_dir(_dir):
		...
		LOG_PATH                 = _dir + 'log.txt'
		NETWORK_BACKUP_DIR       = _dir + 'bk/'
		LOSS_LOG_PATH            = _dir + 'loss_log.txt'
		VALIDATION_LOG_PATH      = _dir + 'val_log.txt'
		NEURAL_NETWORK_SAVE_FILE = _dir + 'nn.dat'
		E_VS_V_FILE              = _dir + 'ev.txt'
		GROUP_ERROR_FILE         = _dir + 'group_error.txt'

	def reconfig_log(group):
		Util.init(LOG_PATH)
		log("---------- Program Started ----------\n")
		log("Command Line: %s\n"%' '.join(sys.argv))
		Util.LogConfiguration()

		f = open(LOG_PATH + '.group', 'w')
		f.write("%s\n"%group)
		f.write('UNWEIGHTED_NEGATIVE_ERROR   = %s\n'%(str(UNWEIGHTED_NEGATIVE_ERROR)))
		f.write('MAXIMUM_TRAINING_ITERATIONS = %s\n'%(str(MAXIMUM_TRAINING_ITERATIONS)))
		f.write('SUBGROUP_ERROR_COEFFICIENT  = %s\n'%(str(SUBGROUP_ERROR_COEFFICIENT)))
		f.close()

	# Run in UNWEIGHTED_NEGATIVE_ERROR = True mode.

	current_output = current_directory + 'UNWEIGHTED_NEGATIVE_ERROR/'
	os.mkdir(current_output)
				
	# First run with unweighted negative error.
	SUBGROUP_ERROR_COEFFICIENT  = 1.0
	UNWEIGHTED_NEGATIVE_ERROR   = True
	OBJECTIVE_FUNCTION          = 'group-targets'
	MAXIMUM_TRAINING_ITERATIONS = 2

	for group in ordered_group_names:

		current_group_dir = current_output + group + '/'
		os.mkdir(current_group_dir)
		set_output_dir(current_group_dir)
		SUBGROUP_TARGETS = {}
		SUBGROUP_TARGETS[group] = 0.001

		print("Running %s"%group)

		reconfig_log(group)
		TrainNetwork()
		Util.cleanup()

	# Now run through each weight value in
	# UNWEIGHTED_NEGATIVE_ERROR = False mode.


	UNWEIGHTED_NEGATIVE_ERROR   = False

	for w in error_coefficients:
		current_output = current_directory + 'WEIGHT_%f/'%w
		os.mkdir(current_output)

		# First run with unweighted negative error.
		SUBGROUP_ERROR_COEFFICIENT  = w
	...
	program_end = time()
	print("Total Run Time: %.1fs"%(program_end - program_start))
