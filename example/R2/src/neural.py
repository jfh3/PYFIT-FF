...
class NN:
	...
	def __init__(self, lines, SB):
		...
		self.normalize_gi=SB['normalize_gi']
		self.info = info
	...
	def NN_eval(self,x):
		...
		if(self.normalize_gi):
			out=(x.Gis-x.Gis.mean())/x.Gis.std()
		else:
			out=x.Gis

		out=(out).mm(torch.t(self.F(self.submatrices[0])))+torch.t((self.F(self.submatrices[1])).mm(x.M1))
